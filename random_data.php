    ...
    <div class="collapse navbar-collapse" id="navbarColor02">
        <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item active">
                <a class="nav-link" href="random_data.php">Generate Data</a>
            </li>
        </ul>
...
<div class="jumbotron">
    ...
    <form id="form-one" method="POST" action="random_data.php">
        ...
        <div class="item-3">
            ...
            <?php
            $hospital_data = array();
            array_push($hospital_data, 'Providence St. Peter Hospital Olympia', 'Capital Medical Center',
            'Providence Medical Center', 'Providence Centralia Hospital', 'Western State Hospital', 'Mason General Hospital',
            'MultiCare Tacoma General Hospital', 'UW Medical Center - Northwest Seattle Hospital', 'MultiCare Good Samaritan Hospital',
                'St. Joseph Medical Center', 'Willapa Harbor Hospital Campus', 'St. Clare Hospital', 'MultiCare Allenmore Hospital');
            $county_data = array();
            ...
            'Norovirus', 'Polio', 'Mono', 'Herpes Simplex Virus', 'Human Papillomavirus', 'Meningitis', 'West Nile Virus',
            'Rabies', 'Ebola', 'COVID-19', 'Strep Throat', 'Gonorrhea', 'Syphilis', 'Tuberculosis', 'Whooping Cough',
            'Pneumocococal pneumonia', 'Lyme disease', 'Cholera', 'Botulism', 'Tetanus', 'Anthrax', 'Malaria', 'Toxoplasmosis',
            'Trichomoniasis', 'Giardiasis', 'Tapeworm infection');
            $state_data = array('WA');
            $patient_email_data = array();
            array_push($patient_email_data, 'liedra@optonline.net', 'xtang@live.com', 'singer@msn.com',
                        'tromey@me.com', 'attwood@gmail.com', 'augusto@hotmail.com', 'dwheeler@icloud.com', 'bogjobber@verizon.net',
                        'moxfulder@att.net', 'wojciech@att.net', 'djpig@yahoo.com', 'sumdumass@aol.com');
            $symptom_description_data = array();
            array_push($symptom_description_data, 'Chills', 'Fever', 'Paresthesia', 'Light-headed', 'Dizzy',
            'Nauseated', 'Short of Breath', 'Sleepy', 'Sweaty', 'Thirsty', 'Tired', 'Weak', 'Convulsions', 'Deformity',
            'Discharge', 'Dizziness', 'Hypothermia', 'Jaundice', 'Muscle Weakness', 'Pyrexia', 'Swelling', 'Weight Gain',
            'Arrhymia', 'Bradycardia', 'Chest pain', 'Palpitations', 'Dry Mouth', 'Hearing Loss', 'Sore Throat', 'Toothache',
            'Tinnitus', 'Abdominal Pain', 'Bleeding', 'Diarrhea', 'Dysphagia', 'Neck Stiffness', 'Paralysis');

            $first_name_data = array();
            array_push($first_name_data, 'James', 'Mary', 'John', 'Patricia', 'Robert', 'Jennifer', 'Michael',
            'Linda', 'William', 'Elizabeth', 'David', 'Barbara', 'Richard', 'Susan', 'Susan', 'Joseph', 'Jessica', 'Joseph',
            ...
            $medicine_data = array();
            array_push($medicine_data, 'Amoxicillin', 'Azithromycin', 'Amoxicillin', 'Clindamycin', 'Cephalexin',
            'Ciprofloxacin', 'Sulfamethoxazole', 'Trimethoprim', 'Levofloxacin', 'Doxycycline', 'Lisinopril', 'Atorvastatin',
            'Levothyroxine', 'Metformin', 'Amlodipine', 'Metoprolol', 'Omeprazole', 'Simvastatin', 'Losartan', 'Albuterol',
            'Vicodin', 'Simvastatin', 'Metformin', 'Amlodipine');
            ?>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if ($_POST['table'] == 'hospital') {
                    for ($i = 1;  $i < sizeof($hospital_data); $i++) {
                        ...
                        mysqli_query($connection, "INSERT INTO HOSPITAL(hospital_name, total_bed, availability_bed, county, covid_test) 
                        VALUES ('$hosp_var', $total_bed_var, $avail_bed_var, '$county_var', $covid_test_var)");
                    }
                    ...
                    for ($i = 0; $i <= sizeof($infection_data); $i++) {
                        ...
                        mysqli_query($connection, "INSERT INTO INFECTION(infection_name, infection_rate, medication, num_of_infections) 
                        VALUES ('$infection_var', $infection_rate_var, '$medicine_data_var', $num_infections_var)");
                    }
                    ...
                    for ($i = 0;  $i < sizeof($county_data)-1; $i++) {
                        ...
                        mysqli_query($connection, "INSERT INTO LOCATION(county, state, population, num_of_hospital)
                    ...
                    for ($i = 0; $i < sizeof($patient_email_data); $i++) {
                        ...
                        mysqli_query($connection, "INSERT INTO PATIENT(patient_id, sickness_type, severity, duration, age_range, hosp_name, patient_email) 
                        VALUES ($id_counter, '$rand_sickness_type', $rand_severity, $rand_duration, $rand_age_range, '$rand_hosp_name', '$rand_patient_email')");
                    ...
                    for ($i = 0; $i < sizeof($patient_email_data); $i++) {
                        ...
                        mysqli_query($connection, "INSERT INTO USER_INFO(user_id, email, first_name, last_name, county, sex, age, Case_start_data) 
                            VALUES ($id_counter, '$rand_email', '$rand_first_name', '$rand_last_name', '$rand_county', 
                                    '$rand_sex', $rand_age, '$rand_total_year')");
